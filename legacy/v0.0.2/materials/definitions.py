...
def read_dat(fn, channel, domain, single=False):
    if domain.dim == 2.5:
        if channel == 'Ex':
            NX = domain.nz
            NY = domain.ny
...
#=============================== 
def rcxgen(rgb, domain, material):
    """
    ...
    rgbint = int(material.material_list[material.material_list[:,2] == rgb,0])
    z,x = np.where(domain.geometry == rgbint)
    y = np.zeros([len(x)])
    xyz = np.stack([z*domain.dz,y,x*domain.dx], axis = 1)
    df = pd.DataFrame(xyz, columns = ['X', 'Y', 'Z'])
    df.to_csv('receivers.xyz', index = False)
