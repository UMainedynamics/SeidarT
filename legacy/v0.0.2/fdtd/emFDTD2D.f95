...
subroutine electromag_cpml_2d(nx, nz, dx, dz, npoints_pml, src, f0, nstep)
...
real(kind=dp), dimension(nx,nz) :: eps11, sig11, eps13, sig13, eps33, sig33, &
                                  epsilonx, epsilonz, sigmax, sigmaz
...
real(kind=dp), dimension(nx, nz) :: sigh_x,K_x,alpha_x,a_x,b_x, &
                          sige_x_half,K_x_half, alpha_x_half,a_x_half,b_x_half, &
                          sigh_z,K_z,alpha_z,a_z,b_z, &
                          sige_z_half,K_z_half, alpha_z_half,a_z_half,b_z_half

integer :: thickness_PML_x,thickness_PML_z
...
epsilonx(:,:) = eps11 + eps13
epsilonz(:,:) = eps13 + eps33
sigmax(:,:) = sig11 + sig13 
sigmaz(:,:) = sig13 + sig33
...
do it = 1,NSTEP
  ...
  do i = 1,nx-1  
    do j = 1,nz-1
      ...
      memory_dEx_dz(i,j) = b_z(i,j) * memory_dEx_dz(i,j) + a_z(i,j) * value_dEx_dz
      value_dEx_dz = value_dEx_dz/ K_z(i,j) + memory_dEx_dz(i,j)

      ! The rest of the equation needed for agnetic field updates
      value_dEz_dx = ( Ez(i+1,j) - Ez(i,j) )/dx
      memory_dEz_dx(i,j) = b_x(i,j) * memory_dEz_dx(i,j) + a_x(i,j) * value_dEz_dx
      value_dEz_dx = value_dEz_dx/ K_x(i,j) + memory_dEz_dx(i,j)
  ...
  do j = 2,nz-1
    do i = 1,nx-1
      ...
      memory_dHy_dz(i,j) = b_z_half(i,j) * memory_dHy_dz(i,j) + a_z_half(i,j) * value_dHy_dz
      value_dHy_dz = value_dHy_dz/K_z_half(i,j) + memory_dHy_dz(i,j)
  ...
  do j = 1,nz-1
    do i = 2,nx-1
      ...
      memory_dHy_dx(i,j) = b_x_half(i,j) * memory_dHy_dx(i,j) + a_x_half(i,j) * value_dHy_dx
      value_dHy_dx = value_dHy_dx/K_x_half(i,j) + memory_dHy_dx(i,j)
  ...
  call write_image(Ex, nx-1, nz, it, 'Ex')
  call write_image(Ez, nx, nz-1, it, 'Ez')
