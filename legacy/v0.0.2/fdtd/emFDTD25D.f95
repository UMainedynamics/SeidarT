...
real(kind=dp),dimension(nx, ny, nz) :: K_x,alpha_x,a_x,b_x, & 
...
! do i = 1,nx ! Check the coefficients
!   print *, a_x(i, 12, 12), a_y(i,12,12), a_z(i,12,12), alpha_x(i, 12, 12), alpha_y(i,12,12), alpha_z(i,12,12)
! enddo
!---
  ...
  do k = 1,nz-1
    do i = 1,nx-1  
      do j = 1,ny-1
        ...
        memory_dEz_dy(i,j,k) = b_y_half(i,j,k) * memory_dEz_dy(i,j,k) + a_y_half(i,j,k) * value_dEz_dy
        value_dEz_dy = value_dEz_dy/ K_y_half(i,j,k) + memory_dEz_dy(i,j,k)

        ! The rest of the equation needed for agnetic field updates
        value_dEy_dz = ( Ey(i,j,k+1) - Ey(i,j,k) )/dz
        memory_dEy_dz(i,j,k) = b_z_half(i,j,k) * memory_dEy_dz(i,j,k) + a_z_half(i,j,k) * value_dEy_dz
        value_dEy_dz = value_dEy_dz/ K_z_half(i,j,k) + memory_dEy_dz(i,j,k)
  ...
  ! do k = 1,nz-1
    ! do i = 1,nx-1      
      ! do j = 1,ny-1
        ...
        memory_dEx_dz(i,j,k) = b_z(i,j,k) * memory_dEx_dz(i,j,k) + a_z(i,j,k) * value_dEx_dz
        value_dEx_dz = value_dEx_dz/ K_z(i,j,k) + memory_dEx_dz(i,j,k)

        ! The rest of the equation needed for agnetic field updates
        value_dEz_dx = ( Ez(i+1,j,k) - Ez(i,j,k) )/dx
        memory_dEz_dx(i,j,k) = b_x(i,j,k) * memory_dEz_dx(i,j,k) + a_x(i,j,k) * value_dEz_dx
        value_dEz_dx = value_dEz_dx/ K_x(i,j,k) + memory_dEz_dx(i,j,k)

        ! Now update the Magnetic field
        Hy(i,j,k) = daHy*Hy(i,j,k) + dbHy*( value_dEz_dx + value_dEx_dz )
  ...
  do k = 2,nz-1
    do i = 1,nx-1      
      do j = 1,ny-1
        ...
        memory_dEx_dy(i,j,k) = b_y(i,j,k) * memory_dEx_dy(i,j,k) + & 
            a_y(i,j,k) * value_dEx_dy
        value_dEx_dy = value_dEx_dy/ K_y(i,j,k) + memory_dEx_dy(i,j,k)

        ! The rest of the equation needed for agnetic field updates
        value_dEy_dx = ( Ey(i,j,k) - Ey(i+1,j,k) )/dx
        memory_dEy_dx(i,j,k) = b_x(i,j,k) * memory_dEy_dx(i,j,k) + & 
            a_x(i,j,k) * value_dEy_dx
        value_dEy_dx = value_dEy_dx/ K_x(i,j,k) + memory_dEy_dx(i,j,k)
  ...
  do k = 2,nz-1
    do i = 1,nx-1
      do j = 2,ny-1  
        ...
        memory_dHz_dy(i,j,k) = b_y_half(i,j,k) * memory_dHz_dy(i,j,k) + & 
            a_y_half(i,j,k) * value_dHz_dy
        value_dHz_dy = value_dHz_dy/K_y_half(i,j,k) + memory_dHz_dy(i,j,k)

        ! Changed from half to full node positions 
        value_dHy_dz = ( Hy(i,j,k-1) - Hy(i,j,k) )/dz
        memory_dHy_dz(i,j,k) = b_z(i,j,k) * memory_dHy_dz(i,j,k) + &
            a_z(i,j,k) * value_dHy_dz
        value_dHy_dz = value_dHy_dz/K_z(i,j,k) + memory_dHy_dz(i,j,k)
  ...
  ! ! Compute the differences in the y-direction
    do i = 2,nx-1 
      do j = 1,ny-1 
        ...
        memory_dHz_dx(i,j,k) = b_x_half(i,j,k) * memory_dHz_dx(i,j,k) + & 
            a_x_half(i,j,k) * value_dHz_dx
        value_dHz_dx = value_dHz_dx/K_x_half(i,j,k) + memory_dHz_dx(i,j,k)

        value_dHx_dz = ( Hx(i,j,k) - Hx(i,j,k-1) )/dz ! this is ny-1 length vector
        memory_dHx_dz(i,j,k) = b_z_half(i,j,k) * memory_dHx_dz(i,j,k) + &
            a_z_half(i,j,k) * value_dHx_dz
        value_dHx_dz = value_dHx_dz/K_z_half(i,j,k) + memory_dHx_dz(i,j,k)

        Ey(i,j,k) = ( ( 4*caEy(i,k) + caEy(i-1,k) + caEy(i,k-1) )/6) * Ey(i,j,k) + & 
          ( ( 4*cbEy(i,k) + cbEy(i-1,k) + cbEy(i,k-1) )/6 ) * & 
          (value_dHz_dx + value_dHx_dz)
      enddo
    enddo
  enddo 
  ...
  do k = 1,nz-1
    do i = 2,nx-1  
      do j = 2,ny-1
        ...
        memory_dHx_dy(i,j,k) = b_y_half(i,j,k) * memory_dHx_dy(i,j,k) + &
            a_y_half(i,j,k) * value_dHx_dy
        value_dHx_dy = value_dHx_dy/K_y_half(i,j,k) + memory_dHx_dy(i,j,k)

        value_dHy_dx = ( Hy(i,j,k) - Hy(i-1,j,k) )/dx
        memory_dHy_dx(i,j,k) = b_x_half(i,j,k) * memory_dHy_dx(i,j,k) + &
            a_x_half(i,j,k) * value_dHy_dx
        value_dHy_dx = value_dHy_dx/K_x_half(i,j,k) + memory_dHy_dx(i,j,k)
        
        Ez(i,j,k) = ( ( 4*caEz(i,k) + caEz(i-1,k) + caEz(i,k+1) )/6 ) * Ez(i,j,k) + & 
          ( ( 4*cbEz(i,k) + cbEz(i-1,k) + cbEz(i,k+1) )/6 ) * & 
          (value_dHx_dy + value_dHy_dx)
      enddo
    enddo
  enddo
