...
class FDTDImage:
    ...
    def getprjvals(self):
        ...
        nx = int(domain.nx) + 2*cpml
        nz = int(domain.nz) + 2*cpml
        dx = float(domain.dx)
        dz = float(domain.dz)
        ...
        if self.channel == 'Ex' or self.channel == 'Ez':
            self.srcx = float(electromag.x)/dx + cpml + 1
            self.srcz = float(electromag.z)/dz + cpml + 1
        else:    
            self.srcx = float(seismic.x)/dx + cpml + 1
            self.srcz = float(seismic.z)/dz + cpml + 1
    ...
    def sliceplot(self, dat, axscale, sliceaxes, alpha = 0.3, papercolumnwidth = 7.2):
        ...
        if sliceaxes == 'xy': # dat dims will be y then x
            figuredims = [axscale[1]*papercolumnwidth, axscale[2]*papercolumnwidth]
            exaggeration = axscale[2]/axscale[1]
        elif sliceaxes == 'xz': # Dims will be z then x
            figuredims = [axscale[0]*papercolumnwidth, axscale[2]*papercolumnwidth]
            exaggeration = axscale[2]/axscale[0]
        else:
            figuredims = [axscale[0]*papercolumnwidth, axscale[1]*papercolumnwidth]
